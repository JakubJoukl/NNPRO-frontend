...
export function CreateConversationFormUI({selectedContact, setSelectedContact, status, onSubmit}) {
    ...
    return (<div className={"w-full 2xl:w-[50rem] lg:w-[43rem] space-y-3"}>
        <div className={"flex flex-row items-center justify-start w-full"}>
            ...
            <Typography variant="h6" className={"text-center !flex-grow"}>
                Begin new conversation with <Typography color={"primary"} variant={"span"}>{<b>{selectedContact.username}</b>}</Typography>.
            </Typography>
        </div>
        ...
        />
        <Button disabled={status.callInProgress} size="large" variant="outlined"
                onClick={() => {
